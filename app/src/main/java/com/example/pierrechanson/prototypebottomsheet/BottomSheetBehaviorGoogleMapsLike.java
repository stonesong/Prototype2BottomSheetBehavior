...
//            mCallback.onStateChanged(bottomSheet, state);
    ...
    /**    if (yvel < 0) { // Moving up
            if (getMajorState() == STATE_COLLAPSED) {
                top = mMiddleMinOffset;
            ...
            Pair<Integer, Integer> stateResult = getClosestState(currentTop);
            top = stateResult.first;
            targetState = getState(stateResult.second);
        } else {
            if (getMajorState() == STATE_EXPANDED) {
                top = mMiddleMinOffset;
                targetState = STATE_MIDDLE;
            } else {
                top = mMaxOffset;
